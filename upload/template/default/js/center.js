...
$(function() {
	var _hideClass = '_center' + '_hidden_' + randomString(16);
	//小屏端
	if($(window).width() < 1000) {
		$('#center-body').addClass('pk-hide');
		$('#center-body-backbar>i:eq(0)').on('click', function() {
			$('#center-nav').removeClass('pk-hide');
			$('#center-body,#center-body-backbar').addClass('pk-hide');
			//显示其他元素
			var a = $('body>.' + _hideClass);
			for(var i = 0; i < a.length; i++) {
				$(a[i]).removeClass('pk-hide ' + _hideClass);
			}
		});
	}
	//个人中心导航驱动
	$('#center_nav>li').on('click', function() {
		var This = $(this);
		var _v = This.data('v');
		if(This.hasClass('pk-active') && $(window).width() > 1000) {
			return false;
		}
		...
		if($(window).width() < 1000) {
			//小屏端
			$('#center-nav').addClass('pk-hide');
			$('#center-body,#center-body-backbar').removeClass('pk-hide');
			$('#center-body-backbar>span:eq(0)').html(This.text());
			//隐藏其他元素
			var a = $('body>*:not(#center-maindivbox,.pk-hide)');
			for(var i = 0; i < a.length; i++) {
				if($(a[i]).find('#center-maindivbox').length) {
					continue;
				}
				$(a[i]).addClass('pk-hide ' + _hideClass);
			}
		}
	});
	//动态加载事件
	...
	//显示未读消息
	var _noreadcount = Cnum($_USER['MESSAGE_UNREADCOUNT']);
	if(_noreadcount) {
		$('#center_nav>li.message>span').append('<i class="pk-text-danger pk-text-xs" style="font-style:normal">（' + _noreadcount + '条未读）</i>');
		//绑定清除消息未读提示
		$('#center_nav>li.message').on('click', function() {
			if(_noreadcount) {
				$.getJSON('index.php?c=center&type=readmessage', function(data) {
					if(data['state'] == 'ok') {
						$('#center_nav>li.message>span>i').remove();
						_noreadcount = 0;
					}
				});
			}
