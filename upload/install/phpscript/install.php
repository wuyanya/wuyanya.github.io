...
if ($mysql_type != 'sqlite' && (!$mysql_location || !$mysql_username || !$mysql_database)) {
	ExitJson("请填写正确的数据库信息");
}
if (!$mysql_prefix || !$mysql_charset || !InArray('mysql,sqlite', $mysql_type)) {
	ExitJson("请填写正确的数据库信息");
}
...
if ($mysql_type == 'sqlite') {
	if (PHP_VERSION_ID < 70000) {
		ExitJson('SQLite数据库支持需要PHP>=7.0，请先切换您的主机PHP版本后再操作');
	}
	$rn = CreateRandomString(rand(16, 32));
	$mysql_location = 'database/' . $rn . '.db';
	if (!copy($mp . 'install/mysqldata/database.db', $mp . $mysql_location)) {
		ExitJson('SQLite数据库创建失败');
	}
}
...
try {
	switch ($mysql_type) {
		case 'mysql' :
			$_G['PDO'] = new PDO("{$mysql_type}:host={$mysql_location}", $mysql_username, $mysql_password . '', array(PDO::MYSQL_ATTR_INIT_COMMAND => $mysql_charset));
			$r = sqlQuery("SHOW DATABASES LIKE '{$mysql_database}'", 1);
			if (!$r) {
				//创建数据库
				$r = sqlQuery("CREATE DATABASE `{$mysql_database}`");
				if (!$r) {
					ExitJson('err1:' . sqlError());
				}
			}
			break;
		case 'sqlite' :
			$_G['PDO'] = new PDO("{$mysql_type}:{$mp}{$mysql_location}");
			break;
		default :
			ExitJson('未知类型的数据库');
			break;
