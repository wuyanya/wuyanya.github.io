...
function sendmail($mailto, $mailtitle, $mailcontent, $timeout = false) {
	...
	//SMTP服务器
	$smtpserverport = Cnum($_G['SET']['APP_SYSTEMEMAIL_PORT']) ? $_G['SET']['APP_SYSTEMEMAIL_PORT'] : 25;
	//发送给谁
	...
	$smtp -> debug = false;
	//是否显示发送的调试信息
	$r = $smtp -> sendmail($mailto, $smtpuser, strip_tags($mailtitle), $mailcontent, 'HTML');
	if ($r !== TRUE) {
		if ($_G['SET']['APP_SYSTEMEMAIL_DEBUG']) {
			NewMessage(1, '邮箱调试信息：' . print_r($r, TRUE), 0, 2);
		}
		return FALSE;
	}
	return TRUE;
