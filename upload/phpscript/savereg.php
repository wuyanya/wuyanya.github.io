...
for ($rf = 0; $rf < 1; $rf++) {
	...
	if ($regarray['data'] && $_G['SET']['REGUSERDATA']) {
		$d = json_decode($regarray['data'], TRUE);
		$d2 = json_decode($_G['SET']['REGUSERDATA'], TRUE);
		$regarray['data'] = json_encode(array_merge($d, $d2));
	} else {
		if($_G['SET']['REGUSERDATA']){
			$regarray['data'] = $_G['SET']['REGUSERDATA'];
		}
	}
